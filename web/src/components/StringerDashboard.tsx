...
export const StringerDashboard = () => {
  ...
  const [customerFilters, setCustomerFilters] = useState({ name: '', numericId: '', originClub: '', professorId: '', email: '', phone: '', stringingPoint: '' });
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px' }}>
                    ...
                    <div>
                      ...
                      <select value={customerFilters.originClub} onChange={e => setCustomerFilters({...customerFilters, originClub: e.target.value})} style={{ width: '100%', padding: '12px', borderRadius: '8px', border: 'none', background: 'rgba(0,0,0,0.1)', color: 'white' }}>
                        <option value="">Todos</option>
                        {(appSettings.clubs || []).map((c: string) => <option key={c} value={c}>{c}</option>)}
                      </select>
                    </div>
                  </div>
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px' }}>
