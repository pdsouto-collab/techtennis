      ...
      {selectedProfessorId ? (
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="glass-panel" style={{ background: 'var(--bg-panel-solid)', borderRadius: '24px', overflow: 'hidden' }}>
          ...
          <div style={{ display: 'flex', gap: '32px', padding: '24px 40px', borderBottom: '1px solid #E5E7EB', background: '#FFFFFF' }}>
            <div onClick={() => setActiveTab('agenda')} style={{ fontSize: '18px', fontWeight: activeTab === 'agenda' ? 800 : 600, color: activeTab === 'agenda' ? '#483375' : '#60A5FA', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '8px' }}>
              <Calendar size={20} /> Agenda
            </div>
            <div onClick={() => setActiveTab('students')} style={{ fontSize: '18px', fontWeight: activeTab === 'students' ? 800 : 600, color: activeTab === 'students' ? '#483375' : '#60A5FA', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '8px' }}>
              <Users size={20} /> Meus Alunos
            </div>
            <div onClick={() => setActiveTab('reports')} style={{ fontSize: '18px', fontWeight: activeTab === 'reports' ? 800 : 600, color: activeTab === 'reports' ? '#483375' : '#60A5FA', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '8px' }}>
                    ...
                    students.filter(s => s.professorId === selectedProfessorId).map(student => (
                      <div key={student.id} style={{ background: 'white', borderRadius: '16px', padding: '24px', boxShadow: '0 4px 12px rgba(0,0,0,0.05)', display: 'flex', flexDirection: 'column', gap: '12px' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                          <h3 style={{ fontSize: '18px', fontWeight: 700, margin: 0, color: '#2D1E4B' }}>{student.name}</h3>
                          ...
                          return (
                            <tr key={cls.id} style={{ borderBottom: '1px solid #F3F4F6', background: cls.status === 'completed' ? 'rgba(16,185,129,0.05)' : cls.status.includes('cancelled') ? 'rgba(239,68,68,0.05)' : cls.status === 'rain' ? 'rgba(245,158,11,0.05)' : 'white' }}>
                              <td style={{ padding: '16px 24px' }}>
                                <div style={{ fontWeight: 700, color: '#2D1E4B' }}>{cls.date.split('-').reverse().join('/')}</div>
                                <div style={{ fontSize: '13px', color: '#6B7280' }}>{cls.timeStart} - {cls.timeEnd}</div>
                              </td>
                              <td style={{ padding: '16px 24px', fontWeight: 600, color: '#2D1E4B' }}>{student?.name || 'Desconhecido'}</td>
                      ...
                      <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left' }}>
                        ...
                        <tbody>
                          {classes.filter(c => c.professorId === selectedProfessorId && c.date.startsWith(activeReportFilter.month) && (activeReportFilter.student === '' || c.studentId === activeReportFilter.student)).sort((a,b) => new Date(b.date).getTime() - new Date(a.date).getTime()).map(cls => {
                            ...
                            return (
                              <tr key={cls.id} style={{ borderBottom: '1px solid #F3F4F6', opacity: (cls.status !== 'completed' && !cls.willHaveReplacement) ? 0.6 : 1, color: '#2D1E4B' }}>
                                <td style={{ padding: '12px 24px', fontWeight: 600, color: '#2D1E4B' }}>{cls.date.split('-').reverse().join('/')}</td>
                                <td style={{ padding: '12px 24px', color: '#2D1E4B' }}>{student?.name || '-'}</td>
                                <td style={{ padding: '12px 24px', color: '#2D1E4B' }}>{cls.status === 'completed' ? 'Realizada' : cls.status === 'replacement' ? 'Reposição' : cls.status === 'rain' ? 'Chuva' : cls.status.includes('cancelled') ? 'Cancelada' : 'Planejada'}</td>
                                <td style={{ padding: '12px 24px', color: '#2D1E4B' }}>{hours.toFixed(1)}h</td>
                        ...
                        <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
                          {[
                            ...
                            <div 
                              ...
                              style={{ padding: '6px 12px', borderRadius: '100px', cursor: 'pointer', fontSize: '13px', fontWeight: 600, background: selectedDays.includes(day.val) ? '#60A5FA' : 'rgba(255,255,255,0.1)', color: selectedDays.includes(day.val) ? '#2D1E4B' : 'white' }}
                            >
                              {day.label}
                            </div>
                          ))}
