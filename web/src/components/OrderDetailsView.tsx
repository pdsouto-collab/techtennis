          ...
          {jobs.filter(isSameOrder).map((orderJob: any) => {
            ...
            <div key={orderJob.id} style={{ minWidth: '1000px', display: 'grid', gridTemplateColumns: 'minmax(220px, 1.5fr) 1fr 1fr 1fr 1fr 1fr 1fr 240px', alignItems: 'center', padding: '24px 0', borderBottom: '1px solid #F3F4F6' }}>
                ...
                <div style={{ display: 'flex', gap: '6px', justifyContent: 'center' }}>
                  ...
                  <button onClick={() => { import('../utils/printUtils').then(m => m.printLabel(orderJob, 'heart', rackets?.find((r:any) => r.id === orderJob.racketId))); }} style={{ background: '#E5E7EB', border: 'none', width: '32px', height: '32px', borderRadius: '6px', color: '#6B7280', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer' }} title="Imprimir Etiqueta Coração"><Ticket size={16} strokeWidth={3} /></button>
                  <button onClick={() => { import('../utils/printUtils').then(m => m.printLabel(orderJob, 'full', rackets?.find((r:any) => r.id === orderJob.racketId))); }} style={{ background: '#E5E7EB', border: 'none', width: '32px', height: '32px', borderRadius: '6px', color: '#6B7280', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer' }} title="Imprimir Etiqueta"><Printer size={16} strokeWidth={3} /></button>
