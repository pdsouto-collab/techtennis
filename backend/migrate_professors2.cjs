...
async function run() {
  ...
  // First, fetch the professors to map their names to IDs in PROFESSORPROFILE (which UI uses)
  const resProfessors = await client.query('SELECT "id", "name" FROM "ProfessorProfile"');
  const profMap = {};
  ...
  for(let profName in updatesProfessors) {
      ...
      if(!pId) {
          console.warn('⚠️ Professor não encontrado no BD da Vercel (ProfessorProfile): ' + profName + '. Tentando busca parcial...');
  ...
  console.log(`✅ ${pCount} clientes vinculados aos seus respectivos professores em PROFESSOR PROFILE!`);
